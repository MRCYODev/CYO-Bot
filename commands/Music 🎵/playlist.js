...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        if (!searchString || !url) return message.reply(`(prefix)playlist <YouTube Playlist URL | Playlist Name>`, message.channel);
        if (url.match(/^https?:\/\/(www.youtube.com|youtube.com)\/playlist(.*)$/)) {
            try {
                const playlist = await ytpl(url.split("list=")[1]);
                if (!playlist) return message.reply("Playlist not found", message.channel);
                ...
                console.error(error);
                return message.reply("Playlist not found :(", message.channel).catch(console.error);
            }
        } else {
            try {
                var searched = await yts.search(searchString);

                if (searched.playlists.length === 0) return message.reply("Looks like i was unable to find the playlist on YouTube", message.channel);
                ...
                return message.reply("An unexpected error has occurred", message.channel).catch(console.error);
            }
        }
        ...
        async function handleVideo(video, message, channel, playlist = false) {
            ...
            if (!serverQueue) {
                ...
                try {
                    ...
                    return message.reply(`I could not join the voice channel: ${error}`, message.channel);
                }
            } else {
