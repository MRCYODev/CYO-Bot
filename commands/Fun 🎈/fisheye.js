...
module.exports = {
    ...
    aliases: ['fsheye'],
    usage: "(upload image)",
    description: "Making fish eye image",
    timeout: 4000,
    ...
    if (attachments.length === 0) return message.reply({embed: {
        color: 10038562,
        description: `Please upload some images! <a:x_:789158785540948008> `
      }});

    else if (attachments.length > 1) return message.reply({embed: {
        color: 10038562,
        description: `I only can process one image at one time! <a:x_:789158785540948008> `
      }});
