...
module.exports = {
    ...
    description: "mute member from current server",
    timeout: 10000,
    /**
    ...
    run : async(client, message, args) => {
        if(!message.member.hasPermission('MANAGE_MESSAGES')) return message.channel.send({embed: {
            color: 10038562,
            description: "You do not have **MANAGE_MESSAGES** Permissions to mute someone <a:x_:789158785540948008> "
          }})
        const Member = message.mentions.members.first() || message.guild.members.cache.get(args[0])
        if(!Member) return message.channel.send({embed: {
            color: 10038562,
            description: "Please Provide member to mute <a:x_:789158785540948008> "
          }})
